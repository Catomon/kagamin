    ...
    LaunchedEffect(currentLayout) {
        when (currentLayout) {
            LayoutManager.Layout.Default -> {
                windowState.size = DpSize(width = WindowConfig.WIDTH.dp, height = WindowConfig.HEIGHT.dp)
            }

            LayoutManager.Layout.Compact -> {
                windowState.size = DpSize(width = WindowConfig.COMPACT_WIDTH.dp, height = WindowConfig.COMPACT_HEIGHT.dp)
            }

            LayoutManager.Layout.Tiny -> {
                windowState.size = DpSize(width =WindowConfig.TINY_WIDTH.dp, height = WindowConfig.TINY_HEIGHT.dp)
            }
        }
    }
...
private fun WindowScope.AppFrame(kagaminViewModel: KagaminViewModel = get(KagaminViewModel::class.java)) {
    WindowDraggableArea {
        ...
        KagaminApp(
            kagaminViewModel,
            modifier = Modifier.let { if (WindowConfig.isTransparent) it.clip(RoundedCornerShape(12.dp)) else it }
